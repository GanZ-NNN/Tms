...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('admin.programs.sessions.update', [$program, $session]) }}" method="POST">
        @csrf
        @method('PUT') {{-- สำคัญสำหรับการ Update --}}
            ...
            <select id="trainer-select" name="trainer_id" class="w-full px-4 py-2 border rounded-lg" required>
                ...
                @foreach($trainers as $trainer)
                    <option value="{{ $trainer->id }}"
                        {{ old('trainer_id', $session->trainer_id) == $trainer->id ? 'selected' : '' }}>
                        {{ $trainer->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="start_at"
                   value="{{ old('start_at', $session->start_at->format('Y-m-d\TH:i')) }}"
                   class="w-full px-4 py-2 border rounded-lg" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="end_at"
                   value="{{ old('end_at', $session->end_at->format('Y-m-d\TH:i')) }}"
                   class="w-full px-4 py-2 border rounded-lg" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="registration_start_at"
                   value="{{ old('registration_start_at', $session->registration_start_at?->format('Y-m-d\TH:i')) }}"
                   class="w-full px-4 py-2 border rounded-lg" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="registration_end_at"
                   value="{{ old('registration_end_at', $session->registration_end_at?->format('Y-m-d\TH:i')) }}"
                   class="w-full px-4 py-2 border rounded-lg" required>
        </div>
